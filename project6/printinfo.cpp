...
// these are the same Platform and Device global variables from the sample OpenCL code:
// you don't need them declared both times
...
// coment this #define out if you want to use this file as subroutines instead of a main program:
#define MAIN_PROGRAM_TO_TEST
...
SelectOpenclDevice()
{
	...
	// priority order:
	//	1. an nvidia or amd gpu
	//	2. an intel gpu
	//	3. any gpu
	//	4. an intel cpu
	//	(this will need to be adjusted when the intel discrete card, arc, hits the market)
