...
//tx: Truck X starting location in feet
const float TXMAX = 10.0;
const float TXMIN = -10.0;

//txv: Truck X velocity in feet/second
const float TXVMAX = 30.0;
const float TXVMIN = 10.0;

//ty: Truck Y location in feet
const float TYMAX = 55.0;
const float TYMIN = 45.0;

//sv: Snowball overall velocity in feet/second
const float SVMAX = 30.0;
const float SVMIN = 10.0;

//sth(theta): Snowball horizontal launch angle in degrees
const float STHMAX = 90.0;
const float STHMIN = 10.0;

//half-length of the truck
const float HALF_LENGTH = 20.0;
...
//ranf, timeofspeed: helper function
...
main( int argc, char *argv[ ] )
{
        ...
        for( int times = 0; times < NUMTIMES; times++ )
        {
			...
			// does the snowball hit the truck (just check x distances, not height):
                        if( fabs(sbx - truckx) < HALF_LENGTH )
                        {
                                numHits++;
